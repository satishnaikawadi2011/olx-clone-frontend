...
function UpdateProduct() {
	...
	const { title, description, contact, state, city, locality, zip, id } = location.state.product;

	const updateProduct = useStoreActions((actions) => actions.prod.updateProduct);
	const onSubmitHandler = (values) => {
		const update = { ...values, id, contact: values.mobile };
